...
u64 solve(const char *fname, u32 max_conns) {
  ...
  /*
  printf("Read %lld boxes\n", boxes.n);
  for (u64 i = 0; i < boxes.n; i++) {
    Box bp = (Box)boxes.ptr;
    printf("Box %lld: %lld,%lld,%lld,%lld\n", i, bp[i].x, bp[i].y, bp[i].z,
           bp[i].c);
  }
  */
  ...
  /*
  for (u64 i = 0; i < 10; i++)
    printf("Edge %lld -> %lld (%lld)\n", ep[i].i, ep[i].j, ep[i].d);
  */

  u32 *circuit = calloc(boxes.n, sizeof(u32));
  u32 *connected = calloc(boxes.n * boxes.n, sizeof(u32));
  u32 ncircuits = 0, nconns = 0;
  for (u64 n = 0; n < edges.n; n++) {
    Edge_t e = ep[n];
    ...
    if (ci == 0 && cj == 0) {
      ...
      // connected[emin * boxes.n + emax] = 1;
    }

    u32 emin = MIN(e.i, e.j), emax = MAX(e.i, e.j);
    assert(emin != emax);
    nconns += (connected[emin * boxes.n + emax] == 0);
    connected[emin * boxes.n + emax] = 1;
    // nconns += 1;

    if (nconns >= max_conns) break;
  }

  /*
  printf("ncircuits: %u\n", ncircuits);
  for (u64 i = 0; i < boxes.n; i++) printf("circuit %u\n", circuit[i]);
  */
  ...
  free(circuit), free(connected), free(sizes);
